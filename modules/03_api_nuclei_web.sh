...
mod_nuclei() {
  ...
  # FULL POWER MODE - run all templates + headless + network scans
  if [[ "${BH_FULL_POWER:-0}" == "1" ]]; then
    log_info "${YELLOW}FULL POWER MODE ENABLED${NC} — Running all templates"
    
    # Full template coverage (all severities)
    log_info "[1] All CVEs..."
    safe_run "${NUC_BASE[@]}" -tags cve -severity critical,high,medium,low -o "$N/cve_all.txt"

    log_info "[2] All misconfigurations & exposures..."
    safe_run "${NUC_BASE[@]}" -tags misconfig,exposure,token,dump -severity critical,high,medium,low -o "$N/misconfig_exposures_all.txt"

    log_info "[3] Default credentials, panels & takeover..."
    safe_run "${NUC_BASE[@]}" -tags default-login,panel,takeover,subdomain -o "$N/panels_takeover_all.txt"

    log_info "[4] Injections (SSRF, SSTI, LFI, RCE)..."
    safe_run "${NUC_BASE[@]}" -tags ssrf,ssti,lfi,rce -o "$N/injections_all.txt"

    log_info "[5] Cloud & infrastructure..."
    safe_run "${NUC_BASE[@]}" -tags aws,gcp,azure,cloud,s3,docker,kubernetes -o "$N/cloud_all.txt"

    log_info "[6] API, GraphQL & network..."
    safe_run "${NUC_BASE[@]}" -tags graphql,api,rest,dns,ssl,tls -o "$N/api_network_all.txt"

    log_info "[7] Headless browser testing (FULL)..."
    if command -v chromium-browser &>/dev/null || command -v chromium &>/dev/null; then
      safe_run "${NUC_BASE[@]}" -headless -tags headless -severity critical,high,medium,low -o "$N/headless_all.txt"
    else
      log_warn "No Chrome/Chromium — skipping headless"
    fi

    log_info "[8] Fuzzing & workflows..."
    safe_run "${NUC_BASE[@]}" -tags fuzz -o "$N/fuzz_all.txt"
    safe_run "${NUC_BASE[@]}" -w cves -o "$N/workflows_cves.txt"
