...
mod_github_recon() {
  ...
  if cmd_exists trufflehog; then
    ...
    HITS=$(grep -c "Raw" "$GH/trufflehog_github.json" 2>/dev/null || true)
    [[ -z "$HITS" ]] && HITS=0
...
mod_modern_attacks() {
  ...
  head -20 "$LIVE" | while IFS= read -r url; do
    ...
    # ── ACTIVE CSP BYPASS TESTING ───────────────────────
    log_info "Active CSP bypass testing for: $url"
    
    # Test 1: JSONP endpoint exploitation (if CSP allows script-src from APIs)
    if echo "$CSP" | grep -qi "script-src.*api\|script-src.*\*\|unsafe-inline"; then
      JSONP_URL="${url}/api/search?callback=test"
      JSONP_RESP=$(curl -sk --max-time "$TIMEOUT" "$JSONP_URL" 2>/dev/null)
      if echo "$JSONP_RESP" | grep -q "test("; then
        log_finding "CSP Bypass via JSONP: $JSONP_URL"
        echo "[HIGH] CSP Bypass via JSONP: $JSONP_URL" >> "$MA/csp_bypass_active.txt"
      fi
    fi
    
    # Test 2: AngularJS template injection (if CSP allows unsafe-eval or specific CDNs)
    if echo "$CSP" | grep -qi "unsafe-eval\|angular\|cdn.jsdelivr.net\|cdnjs.cloudflare.com"; then
      ANGULAR_PAYLOAD="${url}?q={{constructor.constructor('alert(1)')()}}"
      ANGULAR_RESP=$(curl -sk --max-time "$TIMEOUT" "$ANGULAR_PAYLOAD" 2>/dev/null)
      if echo "$ANGULAR_RESP" | grep -q "alert(1)"; then
        log_finding "CSP Bypass via AngularJS: $ANGULAR_PAYLOAD"
        echo "[CRITICAL] CSP Bypass via AngularJS: $ANGULAR_PAYLOAD" >> "$MA/csp_bypass_active.txt"
      fi
    fi
    
    # Test 3: SVG XSS with data URI (if CSP allows data:)
    if echo "$CSP" | grep -qi "data:"; then
      SVG_PAYLOAD="data:image/svg+xml,<svg onload=alert(1)>"
      # Try to inject via img src or other vectors
      # This would typically be tested via reflected XSS parameters
      echo "[TEST] SVG XSS possible if data: allowed: $url" >> "$MA/csp_bypass_active.txt"
    fi
    
    # Test 4: Service Worker abuse (if CSP allows worker-src or script-src from same origin)
    if echo "$CSP" | grep -qi "worker-src\|script-src.*self\|script-src.*$DOMAIN"; then
      SW_TEST_URL="${url}/sw-test.js"
      SW_RESP=$(curl -sk --max-time "$TIMEOUT" "$SW_TEST_URL" 2>/dev/null)
      if [[ -n "$SW_RESP" ]]; then
        log_finding "Service Worker endpoint found: $SW_TEST_URL"
        echo "[MEDIUM] Service Worker endpoint: $SW_TEST_URL" >> "$MA/csp_bypass_active.txt"
      fi
    fi
    
    # Test 5: WebSocket-based attacks (if CSP doesn't restrict connect-src)
    if ! echo "$CSP" | grep -qi "connect-src"; then
      WS_TEST_URL="${url}/ws-test"
      WS_STATUS=$(curl -sk -o /dev/null -w "%{http_code}" \
        -H "Upgrade: websocket" \
        -H "Connection: Upgrade" \
        --max-time "$TIMEOUT" "$WS_TEST_URL" 2>/dev/null)
      if [[ "$WS_STATUS" == "101" ]]; then
        log_finding "WebSocket without CSP connect-src restriction: $WS_TEST_URL"
        echo "[MEDIUM] WebSocket CSP bypass: $WS_TEST_URL" >> "$MA/csp_bypass_active.txt"
      fi
    fi
  done
