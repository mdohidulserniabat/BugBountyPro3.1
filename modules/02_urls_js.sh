...
mod_urls() {
  ...
  TOTAL=$(cat "$U/all_urls.txt" 2>/dev/null | wc -l || echo 0)
  ...
  log_info "Unique params: $(cat "$U/unique_params.txt" 2>/dev/null | wc -l || echo 0)"
}
...
mod_js_analysis() {
  ...
  TOTAL_JS=$(cat "$JS_FILES" 2>/dev/null | wc -l || echo 0)
  ...
  ENDPOINTS=$(cat "$J/js_endpoints.txt" 2>/dev/null | wc -l || echo 0)
