...
mod_v4_csp_engine() {
  ...
  head -25 "$LIVE" | while IFS= read -r url; do
    ...
    # ── ACTIVE CSP BYPASS TESTING (V4 ENGINE) ───────────
    # Test JSONP endpoints if CSP allows script-src from APIs
    if echo "$csp" | grep -qi "script-src.*api\|script-src.*\*\|unsafe-inline"; then
      ...
      if echo "$JSONP_RESP" | grep -q "test("; then
        v4_emit_finding "high" "CSP Bypass via JSONP" "85" "jsonp_endpoint=$JSONP_URL" "Exploit JSONP endpoint for XSS despite CSP" "csp:jsonp" "csp_bypass"
      fi
    fi
    
    # Test AngularJS template injection
    if echo "$csp" | grep -qi "unsafe-eval\|angular\|cdn.jsdelivr.net\|cdnjs.cloudflare.com"; then
      ...
      if echo "$ANGULAR_RESP" | grep -q "alert(1)"; then
        v4_emit_finding "critical" "CSP Bypass via AngularJS" "95" "angular_payload=$ANGULAR_PAYLOAD" "AngularJS template injection bypasses CSP" "csp:angular" "csp_bypass"
      fi
    fi
    ...
    if echo "$csp" | grep -qi "worker-src\|script-src.*self\|script-src.*$DOMAIN"; then
      ...
      if [[ -n "$SW_RESP" ]]; then
        v4_emit_finding "medium" "Service Worker CSP bypass" "70" "sw_endpoint=$SW_TEST_URL" "Service Worker can be used to bypass CSP restrictions" "csp:sw" "csp_bypass"
      fi
    fi
  done
}
