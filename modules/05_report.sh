...
mod_report() {
  ...
  elif [[ $SCORE -ge 20 ]]; then RISK="HIGH"
  elif [[ $SCORE -ge 5  ]]; then RISK="MEDIUM"
  elif [[ $SCORE -ge 1  ]]; then RISK="LOW"
  else RISK="INFO"; fi
...
<span class="cb $([ $(cat "$OUTPUT_DIR/vulns/nuclei/cve_critical.txt" "$OUTPUT_DIR/vulns/nuclei/cve_high.txt" 2>/dev/null | wc -l || echo 0) -gt 0 ] && echo "" || echo "z")">$(cat "$OUTPUT_DIR/vulns/nuclei/cve_critical.txt" "$OUTPUT_DIR/vulns/nuclei/cve_high.txt" 2>/dev/null | wc -l || echo 0)</span></div>
<div class="cd"><pre>$(cat "$OUTPUT_DIR/vulns/nuclei/cve_critical.txt" "$OUTPUT_DIR/vulns/nuclei/cve_high.txt" 2>/dev/null | head -60 || echo "No critical CVEs found")</pre></div>
</div>
