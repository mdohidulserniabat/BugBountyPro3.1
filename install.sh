...
# ── OS Detection ─────────────────────────────────────
detect_os() {
  # Check for Windows/WSL first
  if [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "win32" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    if command -v wslpath &>/dev/null; then
      # WSL environment - detect Linux distro
      ...
      # Pure Windows - check for package managers
      if command -v choco &>/dev/null; then
        PKG="choco"; INSTALL="choco install -y"
      elif command -v scoop &>/dev/null; then
        PKG="scoop"; INSTALL="scoop install"
      else
        PKG="manual"; INSTALL=""
        warn "Windows detected without package manager. Please install dependencies manually or use WSL."
      fi
    fi
    ...
    if command -v brew &>/dev/null; then
      ...
      PKG="manual"; INSTALL=""
      warn "Homebrew not found on macOS. Please install Homebrew first."
    fi
  elif grep -qi "kali\|debian\|ubuntu" /etc/os-release 2>/dev/null; then
    # Debian/Ubuntu/Kali
    PKG="apt"; INSTALL="sudo apt-get install -y -qq"
  elif grep -qi "centos\|fedora\|rhel" /etc/os-release 2>/dev/null; then
    # RHEL/CentOS/Fedora
    ...
    # Alpine Linux
    PKG="apk"; INSTALL="sudo apk add --no-cache"
  else
    # Fallback to apt
    PKG="apt"; INSTALL="sudo apt-get install -y -qq"
  fi
...
if [[ "$PKG" == "apt" ]]; then
  PKG_NAMES=(
    [curl]=curl [wget]=wget [git]=git [jq]=jq [whois]=whois 
    [dnsutils]=dnsutils [nmap]=nmap [nikto]=nikto [python3]=python3
    [python3-pip]=python3-pip [sqlite3]=sqlite3 [libpcap-dev]=libpcap-dev
    [chromium-browser]=chromium-browser
  )
elif [[ "$PKG" == "yum" ]]; then
  PKG_NAMES=(
    [curl]=curl [wget]=wget [git]=git [jq]=jq [whois]=whois 
    [dnsutils]=bind-utils [nmap]=nmap [nikto]=nikto [python3]=python3
    [python3-pip]=python3-pip [sqlite3]=sqlite [libpcap-dev]=libpcap-devel
    [chromium-browser]=chromium
  )
elif [[ "$PKG" == "apk" ]]; then
  PKG_NAMES=(
    [curl]=curl [wget]=wget [git]=git [jq]=jq [whois]=whois 
    [dnsutils]=bind-tools [nmap]=nmap [nikto]=nikto [python3]=python3
    [python3-pip]=py3-pip [sqlite3]=sqlite [libpcap-dev]=libpcap-dev
    [chromium-browser]=chromium
  )
elif [[ "$PKG" == "brew" ]]; then
  PKG_NAMES=(
    [curl]=curl [wget]=wget [git]=git [jq]=jq [whois]=whois 
    [dnsutils]=bind [nmap]=nmap [nikto]=nikto [python3]=python3
    [python3-pip]=python3 [sqlite3]=sqlite3 [libpcap-dev]=libpcap
    [chromium-browser]=chromium
  )
elif [[ "$PKG" == "choco" ]]; then
  PKG_NAMES=(
    [curl]=curl [wget]=wget [git]=git [jq]=jq [nmap]=nmap
    [python3]=python3 [sqlite3]=sqlite [chromium-browser]=chromium
  )
fi

for pkg in curl wget git jq whois dnsutils nmap nikto python3 \
           python3-pip sqlite3 libpcap-dev chromium-browser; do
  
  # Skip packages not available on Windows/Chocolatey
  if [[ "$PKG" == "choco" ]] && [[ "$pkg" == "dnsutils" || "$pkg" == "whois" || "$pkg" == "libpcap-dev" ]]; then
    warn "Skipping $pkg (not available via Chocolatey)"
    continue
  fi
  
  # Get actual package name for current system
  ACTUAL_PKG="${PKG_NAMES[$pkg]:-$pkg}"
  cmd=$(echo "$ACTUAL_PKG" | sed 's/-browser//;s/-dev//;s/-devel//;s/py3-//')
  
  if ! command -v "$cmd" &>/dev/null; then
    if [[ "$PKG" != "manual" && -n "$INSTALL" ]]; then
      echo -n "  Installing $ACTUAL_PKG... "
      if [[ "$PKG" == "choco" ]]; then
        # Handle Chocolatey special cases
        case "$ACTUAL_PKG" in
          python3) 
            $INSTALL python3 2>/dev/null && ok "$ACTUAL_PKG" || fail "$ACTUAL_PKG" ;;
          *) 
            $INSTALL "$ACTUAL_PKG" 2>/dev/null && ok "$ACTUAL_PKG" || fail "$ACTUAL_PKG" ;;
        esac
      else
        $INSTALL "$ACTUAL_PKG" 2>/dev/null && ok "$ACTUAL_PKG" || fail "$ACTUAL_PKG"
      fi
    else
      fail "$ACTUAL_PKG (manual install required)"
    fi
  else
    ok "$ACTUAL_PKG"
  fi
...
# ── Optional v4 Deep Analysis Tools ──────────────────
step "Optional v4 Deep Analysis Tools"

if ! command -v node &>/dev/null; then
  echo -n "  nodejs... "
  if [[ "$PKG" == "apt" ]]; then
    sudo apt-get install -y -qq nodejs npm 2>/dev/null && ok "nodejs" || fail "nodejs"
  elif [[ "$PKG" == "yum" ]]; then
    sudo yum install -y -q nodejs npm 2>/dev/null && ok "nodejs" || fail "nodejs"
  elif [[ "$PKG" == "brew" ]]; then
    brew install node 2>/dev/null && ok "nodejs" || fail "nodejs"
  else
    fail "nodejs"
  fi
else
...
if command -v npm &>/dev/null; then
  for pkg in esprima tree-sitter tree-sitter-javascript; do
    echo -n "  npm:$pkg... "
    npm install -g "$pkg" 2>/dev/null && ok "$pkg" || fail "$pkg"
  done
else
  warn "npm not found; skipping JS parser packages"
fi

echo -n "  semgrep... "
pip3 install -q semgrep 2>/dev/null && ok "semgrep" || fail "semgrep"
...
for tool in subfinder httpx nuclei dnsx katana gau dalfox ffuf \
            subjack subzy gowitness jq python3 nmap node semgrep; do
